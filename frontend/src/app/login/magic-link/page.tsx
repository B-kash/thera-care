...
export default function MagicLinkPage() {
  ...
  async function onSubmit(e: React.FormEvent) {
    ...
    setPending(true);
    try {
      await apiFetchJson<{ ok: boolean }>("/auth/magic-link/request", {
        method: "POST",
    ...
    <div className="w-full max-w-sm rounded-xl border border-app-border/80 bg-app-elevated/90 p-5 shadow-lg shadow-black/10 backdrop-blur-xl dark:shadow-black/40 sm:p-8">
      ...
      <p className="mt-1 text-sm text-foreground/70">
        {sent
          ...
          : "We will email you a short-lived link if this address is registered."}
      </p>

      {sent ? (
        <p className="mt-6 text-sm text-foreground/80">
          Check your inbox and open the link on this device. Links expire
          quickly for security.
        </p>
      ) : (
        <form className="mt-6 space-y-4" onSubmit={(e) => void onSubmit(e)}>
          ...
          <Button type="submit" className="w-full" disabled={pending}>
            {pending ? "Please wait…" : "Send sign-in link"}
          </Button>
        </form>
      )}
